...
**Billable time tracking** — sitting down starts a blinking LED prompt: pick which of several ongoing projects the time should count against, using a small set of Zigbee buttons (a six-sided smart cube was tried first for project selection, but plain buttons turned out easier to use). Once a project is picked, an hours sensor tracks time against it and rolls into a daily-resetting meter. Getting up doesn't need a button press — the same seat sensor, combined with a short delay and the room's motion sensor, recognizes when you've actually left and stops the clock automatically until you sit back down.
